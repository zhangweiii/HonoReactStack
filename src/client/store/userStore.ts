      ...
      (set, get) => ({
        ...
        fetchUsers: async () => {
          ...
          try {
            ...
            if (!response.ok) {
              throw new Error(i18next.t('users:fetchError', `获取用户失败: ${response.status}`, { status: response.status }))
            }
            const users = await response.json() as User[]
            set({ users, isLoading: false })
          } catch (error) {
            set({
              error: error instanceof Error ? error.message : i18next.t('users:unknownFetchError', '获取用户时发生未知错误'),
              isLoading: false
            })
          }
        },
